...
export default React.memo(
  (props: IAddWarehouseProps) => {
    ...
    const iAreaCascaderComponentProps = React.useMemo(
      () => ({
        deep: 2,
        setAreaInfo,
    ...
    function IAreaCascaderComponentForm() {
      return IAreaCascaderComponent(iAreaCascaderComponentProps);
    }
    return (
      <Modal
      ...
      >
        <Form {...layout} form={form} preserve={false} initialValues={{ isDefault: true }}>
          <Form.Item
            name="name"
            label="仓库名"
            rules={[
          ...
          <Form.Item name="isDefault" label="添加默认仓位" valuePropName="checked">
            <Switch checkedChildren="默认仓位: 1个" disabled unCheckedChildren="无默认仓位" />
          </Form.Item>
        </Form>
      </Modal>
    );
